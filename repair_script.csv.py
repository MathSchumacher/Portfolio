...
new_animation_code = r'''
  ...
  (function initAnimations() {
    ...
    if (toggleTrajetoriaBtn && carouselContainerAbout && resumidaAbout) {
      ...
      toggleTrajetoriaBtn.cloneNode(true); // Remove old listeners hack
      // Better: remove event listener if possible or use new clone
...
try:
    ...
    if len(lines) > 1863:
        ...
        print(f"Truncating to 1863 lines. Dropping {len(lines) - 1863} lines.")
    else:
        clean_lines = lines
        print("File is shorter than 1863 lines (unexpected but ok).")
