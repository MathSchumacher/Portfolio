...
const logPath = path.join(__dirname, 'fix_debug.log');
const sourcePath = path.join(__dirname, 'script.js');
const targetPath = path.join(__dirname, 'script.js'); // Overwrite target
const tempPath = path.join(__dirname, 'script_DEBUG_FIXED.js');
...
log('--- STARTING FIX ' + new Date().toISOString() + ' ---');

const newAnimationCode = `
  // ==================================================
...
try {
  ...
  log(`Read lines: ${lines.length}`);
  
  const cutOff = 1863;
  let cleanLines = lines;
  
  if (lines.length > cutOff) {
    cleanLines = lines.slice(0, cutOff);
    log(`Truncated to: ${cutOff}`);
  }
  
  const content = cleanLines.join('\n') + '\n\n' + newAnimationCode;
  
  log(`Writing to TEMP file: ${tempPath}`);
  fs.writeFileSync(tempPath, content, 'utf8');
  log('Temp file written.');
  
  log(`Renaming TEMP to TARGET: ${targetPath}`);
  
  // Try to rename - safer than write
  if (fs.existsSync(targetPath)) {
      try {
        fs.unlinkSync(targetPath); // Try delete first
        log('Deleted original file.');
      } catch(delErr) {
        log('Could not delete original file (EPERM?): ' + delErr.message);
        // Fallback to Overwrite
        fs.writeFileSync(targetPath, content, 'utf8');
        log('Used Overwrite Fallback.');
        return;
      }
  }
  
  fs.renameSync(tempPath, targetPath);
  log('SUCCESS: Renamed temp to target.');
